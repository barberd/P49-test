...
try:
    while True:
        ...
        threadmap={}
        results={}
        executor = concurrent.futures.ThreadPoolExecutor(max_workers=num_features)
        starttime=datetime.datetime.now()
        for att in featurelist[:num_features]:
            query["Key"]["PK"]["S"]="%s#%s"%(accountid.decode('UTF-8'),att.decode('UTF-8'))
            t=executor.submit(doquery,query)
            threads.append(t)
            threadmap[t]=att
        for t in concurrent.futures.as_completed(threads):
            result = t.result()
            threads.remove(t)
            results[threadmap[t]]=result
        responsetime=(datetime.datetime.now()-starttime).total_seconds()*1000
        executor.shutdown(wait=False)
        if i!=0:
            totalqueryresponsesize=0
            for att in featurelist:
                response=results[att]
                if 'Item' in response:
                    size=len(response['Item']['payload']['B'])
                else:
                    size=0
...
if True:
    ...
    figfilename="ddb-multiget-%i.png"%(threads_per_query)

    pl.figure()
    fig = pl.hist(querytimes,log=True,bins=30)
    pl.title('Query latency %i features parallel GetItem with %i parallel threads each'%(num_features,threads_per_query))
    pl.xlabel('latency')
    pl.ylabel('Frequency')
    pl.xlim(0,200)
    pl.ylim(.90,6000)
    ...
    pl.title('Response Time to Size for %i features parallel GetItem with %i parallel threads each'%(num_features,threads_per_query))
