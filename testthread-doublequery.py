...
try:
    while True:
        ...
        (accountid,plasticid)=getrandomfeature()
        query2["Key"]["accountDataLookupKey"]["S"]=accountid
        query2["Key"]["sortQualifier"]["S"]="sortQualifier=plasticId#"+plasticid
        q=queue.Queue()
        starttime=datetime.datetime.now()
        t=threading.Thread(target=doquery,args=(query2,q))
        t.start()
        ...
        if i!=0:
            ...
            if 'Item' in response:
                r1size=len(response['Item']['async-aggregates-plastic']['B'])
            else:
                r1size=0
                empties+=1
            if 'Item' in response2:
                r2size=len(response2['Item']['async-aggregates-plastic']['B'])
            else:
                r2size=0
                empties+=1
            querysizes.append(r1size+r2size)
